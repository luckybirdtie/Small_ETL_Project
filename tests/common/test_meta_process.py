...
class TestMetaProcessMethods(unittest.TestCase):
    ...
    def test_update_meta_file_no_meta_file(self):
        ...
        date_list_result = list(df_meta_result[MetaProcessFormat.META_DATE_COL.value])
        proc_date_list_result = list(
            pd.to_datetime(df_meta_result[MetaProcessFormat.META_PROCESSED_DATE_COL.value])\
            .dt.date
            )
    ...
    def test_update_meta_file_meta_file_ok(self):
        ...
        meta_key = 'meta.csv'
        meta_content = (
          f'{MetaProcessFormat.META_DATE_COL.value},'
          f'{MetaProcessFormat.META_PROCESSED_DATE_COL.value}\n'
          f'{date_list_old[0]},'
    ...
    def test_update_meta_file_meta_file_wrong(self):
        ...
        meta_key = 'meta.csv'
        meta_content = (
          f'wrong_column,{MetaProcessFormat.META_PROCESSED_DATE_COL.value}\n'
          f'{date_list_old[0]},'
          f'{datetime.today().strftime(MetaProcessFormat.META_PROCESS_DATE_FORMAT.value)}\n'
          f'{date_list_old[1]},'
          f'{datetime.today().strftime(MetaProcessFormat.META_PROCESS_DATE_FORMAT.value)}'
